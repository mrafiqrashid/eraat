...
<a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" bp-button="update" class="btn btn-sm btn-link">
	...
	<i class="la la-edit"></i> <span>{{ trans('backpack::crud.check') }}</span>
</a>
...
<div class="btn-group">
	<a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" class="btn btn-sm btn-link pr-0">
		...
		<span><i class="la la-edit"></i> {{ trans('backpack::crud.check') }}</span>
	</a>
	<a class="btn btn-sm btn-link dropdown-toggle text-primary pl-1" data-toggle="dropdown" data-bs-toggle="dropdown"
